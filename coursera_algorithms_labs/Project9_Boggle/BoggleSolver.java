...
public class BoggleSolver {
    ...
    private static class Node {
        private boolean exist = false;  // if this node is word, record as true
        private Node[] next = new Node[26];//just for  26 uppercase Letters
    }

    // convert  26 uppercase Letters to index
    ...
    // build a Tire tree for dictionary
...
//        }
        ...
        // use treiST method to add word into nodes
        for (String word : dictionary) {
            put(word);
        }
    ...
    private Node get(Node x, String key, int d) {
        ...
        int index = getCharIndex(key.charAt(d));
    ...
    // start from (i,j), use DFS method to explore all the valid path, if there are no node in Path then stop
    public Iterable<String> getAllValidWords(BoggleBoard board) {
        ...
        // get the valid word from start position: (i, j)
        for (int i = 0; i < rows; i++) {
            for (int j = 0; j < cols; j++) {
                marked = new boolean[rows][cols];
                // start from prefix with char at (i,j)
                char c = board.getLetter(i,j);
                if(c=='Q'){
                    String prefix = "QU";
                    Node next_node = root.next[getCharIndex('Q')];
                    if(next_node!=null && next_node.next[getCharIndex('U')]!=null){
                        Node node = next_node.next[getCharIndex('U')];
                        dfs(board, node,2,i,j, prefix,validWords);}
                }else{
                    String prefix = ""+ c;
                    Node next_node = root.next[getCharIndex(c)];
                    if(next_node!=null)
                        dfs(board, next_node, 1, i, j, prefix, validWords);
                }
    ...
    /*
     * node: current node at depth
     * (i, j) : current position
     * prefix: the current prefix at (i,j)
     * */
    private void dfs(BoggleBoard board, Node node, int d, int i, int j, String prefix, TreeSet<String> validWords) {
        if (node == null) return;
        // if the node is a word, then add prefix to validWords;
        if (node.exist && prefix.length()>2) {
            validWords.add(prefix);
            if(prefix.equals("QUESTION")){
                StdOut.println(prefix);
            };
        }
        marked[i][j] = true;
        // the current path until (i, j) is a prefix in any word(node) in dictionary ,then we will continue the dfs
        // otherwise we will not do the dfs along this neighbor
        for (int[] neighbor : neighbors(board, i, j)) {
            int new_i = neighbor[0];
            int new_j = neighbor[1];
            if (!marked[new_i][new_j]) {
                char c = board.getLetter(new_i, new_j);
                String new_prefix;
                Node next_node;
                if(c=='Q') {
                    new_prefix = prefix +"QU";  // build prefix for QU
                    next_node = node.next[getCharIndex('Q')]; // for QU , need to pass to next node with Q and U
                    if(next_node!=null) {
                        next_node =next_node.next[getCharIndex('U')];
                        d =d+2;
                    }
                }
                else{
                    new_prefix = prefix+c;
                    next_node = node.next[getCharIndex(c)];
                    d=d+1;
                }
                if(next_node!=null)
                    dfs(board, next_node, d , new_i, new_j, new_prefix, validWords);
            }
        }
        // remove the marked
        marked[i][j] = false;
    ...
    private boolean validPosition(BoggleBoard board, int i, int j) {
    ...
    private Iterable<int[]> neighbors(BoggleBoard board, int i, int j) {
        ...
        for (int[] direction : directions) {
            ...
            if (validPosition(board, new_i, new_j)) {
                neighbors.add(new int[]{new_i, new_j});
            }
        }
        return neighbors;
    }
    ...
    public int scoreOf(String word) {
        ...
        int freqQU = 0;
        if (word.toUpperCase().contains("QU")) {
            int offset = 0;
            while (offset > 0) {
                offset = word.substring(offset).indexOf("QU");
                if (offset >= 0) freqQU++;
            }
        }
        int wordLength = word.length() + freqQU;
        if (wordLength < 3) return 0;
        else if (wordLength < 5) return 1;
...
////        String dictionaryFile = "./boggle/dictionary-yawl.txt";
...
//            boolean isIndictionary = solver.contains(word);
//            if (!isIndictionary)
//                invalid_count++;
//            else valid_count++;
//            StdOut.println(word );
...
//        }
//        StdOut.println("valid_count: "+valid_count +", invalid_count: "+invalid_count);
//        StdOut.println("Score = " + score);

//        String word = "CROWED";
//        int score = solver.scoreOf(word);
//        StdOut.println("word: " +word +" score= "+ score);

//    }

    public static class BoggleGame extends JFrame {
                ...
                public void run() {
                    ...
                    if (args.length == 0) {
                        ...
                        try {
                            ...
                            System.err.println("Usage: java BoggleSolver.BoggleGame " +
                                               "\nor:    java BoggleSolver.BoggleGame [rows]" +
                                               "\nor:    java BoggleSolver.BoggleGame [rows] [cols]");
                            System.exit(1);
                        }
                    }
                    ...
                    if (rows <= 0 || cols <= 0) {
                        throw new IllegalArgumentException("Rows and columns must be positive" +
                                                                     "\nUsage: java BoggleSolver.BoggleGame " +
                                                                     "\nor:    java BoggleSolver.BoggleGame [rows]" +
                                                                     "\nor:    java BoggleSolver.BoggleGame [rows] [cols]");
                    }
